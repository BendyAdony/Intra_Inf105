...
author: "Bendy,Fabrice,Peter, Darwin"
date: "`r Sys.Date()`"
output:
...
TDJA <- ggplot(data = TDJ, aes(x = CATEGORIES,y=ACHATS,fill = CATEGORIES)) +
            geom_col(just = 0.5)
plot1<-ggplotly(TDJA)

#Graphe du Taux des différents marchés à la vente

TDJV <- ggplot(data = TDJ, aes(x = CATEGORIES,y=VENTES,fill = CATEGORIES)) +
            geom_col(just = 0.5)
plot2<-ggplotly(TDJV)
